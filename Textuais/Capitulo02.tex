...
Este capítulo apresenta o referencial teórico que sustenta a dissertação, organizado em dois movimentos. O primeiro estabelece o domínio em que o problema se manifesta, ao descrever o processo produtivo do fio esmaltado, os mecanismos de rompimento documentados na literatura e os dados que a operação registra. O segundo revisa as famílias de técnicas de apoio à decisão técnica em manufatura, das ferramentas tradicionais às abordagens orientadas a dados, e identifica a lacuna que motiva a abordagem combinada proposta nesta dissertação.

A Seção~\ref{sec:contexto_industrial} descreve a cadeia produtiva de fios esmaltados de cobre e alumínio, das etapas de fundição e laminação do vergalhão até o esmaltamento final. A Seção~\ref{sec:problema_rompimentos} caracteriza o evento de rompimento e seus mecanismos a partir da literatura. A Seção~\ref{sec:dados_disponiveis} inventaria os dados disponíveis para análise e destaca a ausência de rótulo de causa. A Seção~\ref{sec:diagnostico} situa o trabalho na tradição de diagnóstico de falhas em manufatura e expõe os limites das ferramentas clássicas diante de dados sensoriais em alta frequência. As Seções~\ref{sec:anomalias} e~\ref{sec:estatistica_historica} apresentam, respectivamente, a detecção de anomalias em séries operacionais e a análise estatística histórica de falhas, as duas famílias de técnicas que sustentam a proposta. A Seção~\ref{sec:hibridos_xai} discute estratégias de integração das duas famílias e o papel da explicabilidade em ambiente produtivo, apontando a lacuna que esta dissertação preenche.
