...
\documentclass[
	...
	% fleqn,				% equações alinhadas a esquerda (UDESC/CCT); removido para centralizar equações
	]{abntex2}
...
% -----------------------------------------------------------------
\titulo{WireSense: Ferramenta Híbrida de Apoio à Decisão para Inferência da Provável Origem de Rompimentos em uma Fábrica de Fios}%

\autor{Vitor {}Melchioretto}%
\orientador{Mauricio Aronne {}Pillon}
%\coorientador{Nome do Coorientador {}Sobrenome}% sem coorientador
...
\preambulo{Dissertação apresentada ao Programa de Pós--Graduação em Computação Aplicada do Centro de Ciências Tecnológicas da Universidade do Estado de Santa Catarina, como requisito parcial para a obtenção do grau de Mestre em Computação Aplicada.}
...
%\include{PreTextuais/Errata}				% Elemento Opcional (omitido)
\include{PreTextuais/FolhadeAprovacao}		% Elemento Obrigatório
%\include{PreTextuais/Dedicatoria}			% Elemento Opcional (omitido)
\include{PreTextuais/Agradecimentos}		% Elemento Opcional
\include{PreTextuais/DeclaracaoIA}			% Declaracao sobre uso de IA generativa
...
\include{Textuais/Capitulo03}	% Revisão Sistemática da Literatura
\include{Textuais/Capitulo04}	% Proposta (WireSense)
\include{Textuais/Capitulo05}	% Experimentação
\include{Textuais/Capitulo06}	% Considerações Parciais
...
%\include{PosTextuais/Glossario}				% Elemento Opcional (omitido)
%\include{PosTextuais/Apendices}				% Elemento Opcional (omitido)
%\include{PosTextuais/Anexos}				% Elemento Opcional (omitido)
%\include{PosTextuais/IndiceRemissivo}		% Elemento Opcional (omitido)
